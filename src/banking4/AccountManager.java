package banking4;

import java.util.HashSet;
import java.util.InputMismatchException;
import java.util.Scanner;

public class AccountManager implements MenuChoice {
	public static Scanner sc = new Scanner(System.in);
	static HashSet<Account> set = new HashSet<Account>();
		
	
	public static void showMenu() {
		System.out.println("===Menu===\n1. 계좌개설 \n2. 입금 \n3. 출금 \n4. 전체계좌정보출력 \n5. 계좌정보 삭제\n6. 프로그램 종료\n선택 : ");
	}
	
	public void makeAccount() {
		String iaccNum, iname;
		int ibal, binter;
		char grade;
		System.out.println("==신규계좌개설==\n1. 보통계좌 \n2. 신용계좌\n선택 : ");
		int choice2 = 0;
		try {
		choice2 = sc.nextInt();
		if(choice2<1||choice2>2) {
			System.out.println("범위가 잘못되었습니다. \n1~2사이의 숫자믄 입력하세요.");
			return;
		}
		}catch(InputMismatchException e) {
			System.out.println("[예외] 숫자만 입력.");
			sc.nextLine();
			return;
		}
				
		sc.nextLine();
		System.out.println("계좌번호 : ");
		iaccNum = sc.nextLine();
		System.out.println("이름 : ");
		iname = sc.nextLine();
		System.out.println("잔고 : ");
		ibal = sc.nextInt();
		System.out.println("기본이자%(정수로 입력) : ");
		binter = sc.nextInt();


		switch(choice2) {
		case 1 :
			Account addNAcc = new NormalAccount(iaccNum, iname, ibal, binter);
			if(set.add(addNAcc)==false) {
				System.out.println("중복계좌가 발견. \n갱신? (Y/N)");
				char b = sc.next().charAt(0);
				switch(b) {
				case 'Y': case 'y': 
					set.remove(addNAcc);
					set.add(addNAcc);
					System.out.println(" 갱신 완료.");								
					break;
				case 'N': case 'n': 
					System.out.println("계좌개설 취소.");
					return;
				default : System.out.println(" Y 또는 N 만 입력가능. ");
				}
			}
			else set.add(addNAcc);
			break;
		case 2 : 
			System.out.println("신용등급(A,B,C 등급) : ");
			grade = sc.next().charAt(0);
			Account addHAcc = new HighCreditAccount (iaccNum, iname, ibal, binter, grade);
			if(set.add(addHAcc)==false) {
				System.out.println("중복계좌가 발견. \n 갱신? (Y/N)");
				char b = sc.next().charAt(0);
				switch(b) {
				case 'Y': case 'y': 
					set.remove(addHAcc);
					set.add(addHAcc);
					System.out.println("신규계좌개설을 완료.");								
					break;
				case 'N': case 'n': 
					System.out.println("계좌개설이 취소.");
					return;
				default : System.out.println("문자 Y 또는 N 만 입력가능합니다. ");
				}
			}
			else set.add(addHAcc);
		}
		System.out.println("신규계좌 개설이 완료되었습니다.");
	}
	
	public void depositMoney() {
		String iaccNum;
		int iMoney = 0;
		sc.nextLine();
		System.out.println("==입금==\n계좌번호와 입금할 금액을 입력하세요.\n계좌번호 : ");
		iaccNum = sc.nextLine();

		System.out.println("입금액 : ");
//		while(true) {
		try{
			iMoney = sc.nextInt();
			if(iMoney<0||iMoney%500!=0) {
				System.out.println("500원 단위의 양수만 입금 가능.");
			}else {
				for(Account i:set) {
					if(i.getAccNum().equals(iaccNum)) {
						i.plusMoney(iMoney);
						System.out.println("입금 완료.");
						break;
					}else System.out.println("--- 입력하신 계좌가 없습니다 ---");
				}
			}
		}catch(InputMismatchException e) {
			System.out.println("[예외] 숫자만 입력하세요.");
			sc.nextLine();
		}
//		}
	}
	
	public void withdrawMoney() {
		String iaccNum;
		int iMoney = 0;
		sc.nextLine();
		System.out.println("***출금***\n계좌번호와 출금할 금액을 입력하세요.\n계좌번호 : ");
		iaccNum = sc.nextLine();
		
		System.out.println("출금액 : ");
//		while(true) {
		try{
			iMoney = sc.nextInt();
			if(iMoney<0||iMoney%1000!=0) {
				System.out.println("1000원 양수만 출금 가능.");
			}else {
				for(Account i:set) {
					if(i.getAccNum().equals(iaccNum)) {
						if(iMoney<=i.getBal()) {
							i.minusMoney(iMoney);
							System.out.println("출금 완료.");
							break;
						}else {
							System.out.println("잔고 부족. \n금액전체를 출금할까요? (Y/N)");
//							while(true) {
							char a = sc.next().charAt(0);
							switch(a) {
							case 'Y': case 'y': 
								System.out.println(i.getBal()+" 원 (잔액) 출금 완료.");
								i.setBal(0);									
								break;
							case 'N': case 'n': 
								System.out.println("출금 취소.");
								return;
							default : System.out.println(" Y/N 만 입력하세요. ");
							}
//							}
						}
					}else System.out.println("=계좌가 없어요=");
				}
			}
		}catch(InputMismatchException e) {
			System.out.println("[예외] 숫자만 입력하세요.");
			sc.nextLine();
		}
//		}
	}
	
	public static void showAccInfo() {
		System.out.println("===계좌정보출력===");
		for(Account i:set) {
			
			i.showAccInfo();
			
		}		
		System.out.println("전체계좌정보 출력이 완료되었습니다.");
	}
	
	public static void deleteAcc() {
		String iaccNum;
		sc.nextLine();
		System.out.println("==계좌삭제==\n삭제할 계좌번호를 입력하세요.\n계좌번호 : ");
		iaccNum = sc.nextLine();
		for(Account i:set) {
			if(i.getAccNum().equals(iaccNum)) {
				set.remove(i);
				System.out.println(iaccNum+" 계좌를 삭제하였습니다.");
			}else System.out.println("=== 입력하신 계좌가 없습니다 ===");
		}		
	}
}
